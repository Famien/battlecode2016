...
public class RobotPlayer {
	 ...
	 * TODO Turret distribution
	 * TODO  make some players determined
	 * TODO  zombie logic
	 * TODO  patch: try to move method destination rather than direction
	 * 
	 * new stratergy: at least 1 station archon, surrounded by turrets, teams of soldiers / archons collected parts via zombie dens or spare parts, 
	 */
	...
	static RobotType[] buildListLeaderFort = new RobotType[]
	...
	private static void tryToAttack() throws GameActionException {
		...
		if (allEnemies.length > 0) {
			if(!archonFound){
				if (rc.isWeaponReady()) {
					rc.setIndicatorString(0," there is an enemy, and no archon found, so I should be trying to attack");
				...
				for(RobotInfo r:allEnemies){
					...
					if(r.type==RobotType.ZOMBIEDEN){//IF we found an archon, ignore zombie dens TODO anyting else to ignore?
						//continue;TODO don't ignore dens right?
		...
		rc.setIndicatorString(0, "i really sensed no enemies???");
	...
	private static boolean chooseAndBuild(boolean zombiesAround) throws GameActionException {
		if(!archonFound && rc.getRoundNum() > 200){
