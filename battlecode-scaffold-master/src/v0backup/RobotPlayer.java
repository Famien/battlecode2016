...
//		}
		...
		//
		//		}
		if(leader && (rc.getID() + rc.getRoundNum()) % 8 ==0){//TODO don't keep broadcasting so much
			sendArchonInfo();
		}
...
//			}
	...
	private static boolean chooseAndBuild(boolean zombiesAround) throws GameActionException {
		...
		if (zombiesAround || (rc.getRoundNum() + rc.getID() )%5 ==0 	){
			if(buildWherePossible(RobotType.GUARD)){
				return true;
			}
		}
		else if ((rc.getRoundNum()+ rc.getID() ) %7 ==0){
		...
		else if((rc.getRoundNum()+ rc.getID() ) % 11 ==0){
			if(buildWherePossible(RobotType.TURRET))return true;
		}

		else if ((rc.getRoundNum()+ rc.getID() ) %13 == 0||( (rc.getRoundNum()+ rc.getID() ) %3 == 0 ) && !archonFound){
