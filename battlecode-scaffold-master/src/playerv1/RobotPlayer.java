 ...
 * version highlights
 * 
 * function code for turrets and vipers
 *
 */
public class RobotPlayer {
	...
	public static int enemyArchonID;
	...
	static RobotType[] buildListScout = new RobotType[]{
			RobotType.GUARD,
			RobotType.SCOUT,RobotType.SOLDIER,RobotType.SOLDIER,RobotType.TURRET,RobotType.SCOUT};
...
//		}
	...
	private static void archonSignaling() throws GameActionException {
		...
		//
		//		}
		if(leader && (rc.getID() + rc.getRoundNum()) % 11 ==0){//TODO don't keep broadcasting so much
			sendArchonInfo();
		}
	...
	private static void sendArchonInfo() throws GameActionException {
		...
		if(archonFound ){
			...
			else if ((rc.getRoundNum()+ rc.getID())% 8 ==0){
			...
			rc.broadcastMessageSignal(MOVE_X, loc.x, 1000);
			rc.broadcastMessageSignal(MOVE_Y, loc.y, 1000);
		}
	...
	private static boolean chooseAndBuild(boolean zombiesAround) throws GameActionException {
		...
		if (buildWherePossible(buildList[rnd.nextInt(buildList.length)]))return true;
